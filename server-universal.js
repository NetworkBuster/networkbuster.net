...
try {
  ...
  console.warn(
    "⚠️  helmet module not found - continuing without security headers",
  );
}
...
try {
  ...
  console.warn(
    "⚠️  cors module not found - continuing without CORS middleware",
  );
}
...
const allowedOrigins = (
  process.env.CORS_ORIGINS || "https://networkbuster.net,http://localhost:3000"
)
  .split(",")
  .map((o) => o.trim());
...
const challengeOverlayPath = path.join(
  __dirname,
  "challengerepo/real-time-overlay/dist/index.html",
);

// AI Robot endpoint using Azure OpenAI (set AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_KEY, AZURE_OPENAI_DEPLOYMENT)
app.post("/api/robot", async (req, res) => {
  const {
    prompt = "Analyze lunar recycling and space materials. Summarize risks and opportunities.",
  } = req.body || {};
  ...
  if (!endpoint || !apiKey || !deployment) {
    return res.status(500).json({
      error:
        "Azure OpenAI not configured. Set AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_KEY, AZURE_OPENAI_DEPLOYMENT.",
    });
  }
    ...
    messages: [
      {
        role: "system",
        content:
          "You are NetBot, an expert in recycling, lunar regolith processing, and space materials.",
      },
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to call Azure OpenAI", details: err.message });
  }
});
