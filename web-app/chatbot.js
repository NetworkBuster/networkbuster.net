...
class NetworkBusterChatbot {
  ...
  addCommandSuggestions(userInput) {
    ...
    suggestionDiv.innerHTML =
      '<b>Relevant Commands:</b><ul style="margin:8px 0 0 16px;">' +
      matches
        .map(
          (cmd) =>
            `<li><b>${cmd.name}:</b> <code>${cmd.command}</code><br><span style='font-size:0.95em;color:#8b949e;'>${cmd.description}</span></li>`,
        )
        .join("") +
      "</ul>";
  ...
  init() {
    ...
    this.addMessage(
      "bot",
      this.getRandomResponse(KNOWLEDGE_BASE.greetings.responses),
    );
  }
  ...
  isQuestion(text) {
    return (
      SENTIMENT_WORDS.question.some((word) => text.startsWith(word)) ||
      text.endsWith("?")
    );
  }
