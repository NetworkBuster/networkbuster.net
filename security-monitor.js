...
const THREAT_PATTERNS = {
  sqlInjection:
    /(\bunion\b.*\bselect\b|\bor\b.*1\s*=\s*1|;.*drop\b|<script|javascript:|onerror=)/i,
...
function updateSecurityStatus() {
  const now = Date.now();
  const recentThreats = securityState.attemptedHacks.filter(
...
function addToTimeline(event) {
  securityState.timeline.push({
    past:
      securityState.timeline.length > 0
        ? securityState.timeline[securityState.timeline.length - 1]
        : null,
    present: event,
...
function checkIPReputation(ip) {
  ...
  if (securityState.blockedIPs.has(ip)) {
    return {
      blocked: true,
      reason: "Previously flagged for malicious activity",
    };
  }
...
function triggerAmberAlert(threat) {
  ...
  // Log alert
  console.log(
    `🟠 AMBER ALERT: ${alert.type} detected from ${threat.source.ip || "unknown"}`,
  );
...
app.get("/api/security/status", (req, res) => {
  ...
  res.json({
    ...
    statistics: {
      ...
      recentHackAttempts: securityState.attemptedHacks.filter(
...
app.get("/api/security/amber-alerts", (req, res) => {
  const amberAlerts = securityState.alerts.filter(
    (a) => a.level === "AMBER_ALERT",
  );
  res.json({
    status: "🟠",
    alerts: amberAlerts,
...
function analyzeTimeline() {
  ...
  return {
    ...
    trend,
    prediction:
      recent[recent.length - 1]?.present?.future?.prediction || "Unknown",
  };
}
