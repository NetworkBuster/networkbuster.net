...
const AI_BRAIN = {
  ...
  intents: {
    greeting: {
      patterns: [
        /^(hi|hello|hey|greetings|howdy|yo|sup)/i,
    ...
    feedback: {
      patterns: [
        /^(thanks|thank you|great|awesome|cool|nice|good job)/i,
        /^(bad|terrible|wrong|broken)/i,
      ],
      confidence: 0.9,
    },
  },
  ...
  topics: {
    servers: {
      ...
      data: {
        web: {
          port: 3000,
          description: "Main web server with dashboard and control panel",
        },
        api: {
          port: 3001,
          description: "REST API for system data and operations",
        },
        audio: {
          port: 3002,
          description: "Audio streaming and synthesis server",
        },
...
function extractTopics(message) {
  ...
  for (const [topic, config] of Object.entries(AI_BRAIN.topics)) {
    ...
    if (matchCount > 0) {
      foundTopics.push({
        topic,
        relevance: matchCount / config.keywords.length,
      });
    }
  }
  ...
  if (topics.some((t) => t.topic === "servers")) {
    ...
    if (normalized.includes("port") || normalized.includes("what port")) {
      return `NetworkBuster runs on multiple ports:\n${Object.entries(
        ...
        .map(
          ([name, info]) =>
            `• ${name.toUpperCase()}: Port ${info.port} - ${info.description}`,
        )
        .join("\n")}`;
    }
...
function handleCommand(message, topics, entities) {
  ...
  if (normalized.includes("show") || normalized.includes("list")) {
    if (normalized.includes("server")) {
      return `Active servers:\n${Object.entries(AI_BRAIN.topics.servers.data)
        .map(([name, info]) => `• ${name}: localhost:${info.port}`)
        .join("\n")}`;
    }
    if (normalized.includes("feature")) {
      return AI_BRAIN.topics.features.list
        .map((f, i) => `${i + 1}. ${f}`)
        .join("\n");
    }
  }
...
app.get("/api/chat/health", (req, res) => {
  res.json({
    status: "online",
    bot: AI_BRAIN.system.name,
    version: AI_BRAIN.system.version,
  });
});
