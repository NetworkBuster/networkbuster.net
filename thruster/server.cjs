...
function validatePlanOptions(opts) {
  ...
  for (const r of required) {
    if (
      opts[r] === undefined ||
      opts[r] === null ||
      Number.isNaN(Number(opts[r]))
    )
      return { valid: false, error: `missing_or_invalid_${r}` };
  }
  if (Number(opts.initialMass) <= 0)
    return { valid: false, error: "initialMass_must_be_positive" };
  if (Number(opts.propellantAvailable) < 0)
    return { valid: false, error: "propellantAvailable_must_be_nonnegative" };
  if (Number(opts.isp) <= 0)
    return { valid: false, error: "isp_must_be_positive" };
  if (Number(opts.maxG) <= 0)
    return { valid: false, error: "maxG_must_be_positive" };
  return { valid: true };
}
...
app.post("/plan/visualize", requireApiKey, async (req, res) => {
  ...
  try {
    ...
    if (format === "png") {
      const buf = await convertSvgStringToPngBuffer(svg, {
        width: opts.width,
        height: opts.height,
      }).catch(() => null);
      if (buf) {
...
app.post("/visualize/orbit", requireApiKey, async (req, res) => {
  ...
  if (!opts.radius || !opts.period) {
    return res
      .status(400)
      .json({ ok: false, error: "missing_or_invalid_radius_or_period" });
  }
  const format = (req.query.format || req.body.format || "svg").toLowerCase();
  try {
    const {
...
app.post("/plan/optimize", requireApiKey, (req, res) => {
  ...
  try {
    let optimized = null;
    if (method === "heuristic") {
      optimized = planOptimizedMultiSegmentHeuristic(opts, {
        cost,
        steps: Number(req.body.steps || req.query.steps || 12),
        iterations: Number(req.body.iterations || req.query.iterations || 2000),
      });
    } else if (method === "continuous") {
      optimized = planOptimizedMultiSegmentContinuous(opts, {
        cost,
        maxIter: Number(req.body.maxIter || req.query.maxIter || 400),
      });
    } else {
      optimized = planOptimizedMultiSegment(opts, {
        cost,
        steps: Number(req.body.steps || req.query.steps || 6),
      });
    }
...
app.post("/plan/separate", requireApiKey, async (req, res) => {
  ...
  const driftSeconds = Number(
    body.driftSeconds || body.drift_seconds || body.drift || 0,
  );
  if (!driftSeconds || driftSeconds <= 0)
    return res
      .status(400)
      .json({ ok: false, error: "missing_or_invalid_driftSeconds" });
  const onlyIfEven =
    body.onlyIfEven === undefined ? true : String(body.onlyIfEven) === "true";
  const separationDeltaV = Number(
    body.separationDeltaV || body.separation_delta_v || 0,
  );
  const detachedMass =
    body.detachedMass !== undefined ? Number(body.detachedMass) : undefined;
  const notifyWebhook =
    body.notifyWebhook || body.notify_webhook || body.notify || null;

  const v = validatePlanOptions(opts);
  if (!v.valid) return res.status(400).json({ ok: false, error: v.error });

  try {
    const result = await separateAfterDrift(opts, {
      driftSeconds,
      onlyIfEven,
      separationDeltaV,
      detachedMass,
...
app.post("/admin/request-access", (req, res) => {
  try {
    const body = Object.assign({}, req.body || {}, req.query || {});
    if (!body.githubUser || !body.publicKey)
      return res
        .status(400)
        .json({ ok: false, error: "githubUser_and_publicKey_required" });
    const r = admin.requestAccess({
      githubUser: String(body.githubUser),
...
app.post("/admin/approve", requireAdminKey, (req, res) => {
  try {
    ...
    const r = admin.approveRequest(
      String(body.id),
      req.headers["x-admin-user"] || "web-admin",
    );
...
if (require.main === module) {
  ...
  app.listen(port, () =>
    console.log(`Thruster planner API listening on ${port}`),
  );
}
