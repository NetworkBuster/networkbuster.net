...
const server = child.spawn(
  ...
  [require.resolve("../../thruster/server.cjs")],
  {
    stdio: "inherit",
    detached: true,
    env: Object.assign({}, process.env, { THRUSTER_ADMIN_KEY: "adminkey123" }),
  },
);
...
(async () => {
  try {
    ...
    // request access
    const reqBody = {
      githubUser: "test-admin",
      publicKey: "ssh-rsa AAAAB3TestKey",
      reason: "integration test",
    };
    let res = await fetch(`${base}/admin/request-access`, {
      method: "POST",
    ...
    // approve
    res = await fetch(`${base}/admin/approve`, {
      method: "POST",
      headers: {
        "x-admin-key": "adminkey123",
    ...
    if (!apr.ok) {
      console.error("approve failed", apr);
      process.exit(2);
    }
