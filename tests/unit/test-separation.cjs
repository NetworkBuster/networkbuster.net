...
(async () => {
  ...
  const res = await separateAfterDrift(opts, { driftSeconds: 10, onlyIfEven: false });
  assert(res.separated, 'separation should be performed when onlyIfEven=false and plan feasible');
