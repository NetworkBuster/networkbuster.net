...
const { computeDeltaV, propellantForDeltaV, planBurn, g0 } = require('../../thruster/thrusterPhysics.cjs');
...
async function test() {
    ...
    assert.ok(plan.possible === true, 'plan should be possible with available propellant');
    assert.ok(plan.peakG <= 3 + 1e-6, 'peak g should be within limit');
    assert.ok(plan.propellantUsed <= prop + 1e-6, 'used propellant should match required');
